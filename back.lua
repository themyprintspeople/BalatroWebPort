...
function Back:init(selected_back)
    ...
    self.pos.x = pos.x
    self.pos.y = pos.y
    self.atlas = (self.effect.center.unlocked and self.effect.center.atlas) or 'centers'
end
...
function Back:generate_UI(other, ui_scale, min_dims, challenge)
    ...
    if not back_config.unlocked then
        ...
        elseif name_to_check == 'Buffoon Deck' then
        elseif name_to_check == 'Balatro Plus Deck' then loc_args = {effect_config.dollars, effect_config.hand_size}
        elseif name_to_check == 'Showdown Deck' then loc_args = {effect_config.win_ante}
        end
...
function Back:change_to(new_back)
    ...
    self.pos.x = pos.x
    self.pos.y = pos.y
    self.atlas = self.effect.center.unlocked and new_back.atlas or 'centers'
end
...
function Back:apply_to_run()
    ...
    if self.effect.config.win_ante then
...
function Back:load(backTable)
    ...
    self.atlas = self.effect.center.atlas or 'centers'
