...
function Card:set_sprites(_center, _front)
    ...
    if _center then 
        if _center.set then
            if _center._mikas_native and type(_center.atlas) == 'string' and _center.atlas ~= '' and type(ensure_mikas_native_atlas) == 'function' then
                ensure_mikas_native_atlas(_center.atlas)
            end
            if _center._native_smods and type(_center.atlas) == 'string' and _center.atlas ~= '' and type(ensure_native_smods_atlas) == 'function' then
                ensure_native_smods_atlas(_center.atlas)
            end
            if self.children.center then
                ...
                elseif not self.params.bypass_discovery_center and (_center.set == 'Edition' or _center.set == 'Joker' or _center.consumeable or _center.set == 'Voucher' or _center.set == 'Booster') and not _center.discovered then 
                    local atlas_ref = G.ASSET_ATLAS[_center.atlas or _center.set] or G.ASSET_ATLAS[_center.set] or G.ASSET_ATLAS['Joker'] or G.ASSET_ATLAS['centers']
                    self.children.center = Sprite(self.T.x, self.T.y, self.T.w, self.T.h, atlas_ref, 
                ...
                elseif _center.set == 'Joker' or _center.consumeable or _center.set == 'Voucher' then
                    local atlas_ref = G.ASSET_ATLAS[_center.atlas or _center.set] or G.ASSET_ATLAS[_center.set] or G.ASSET_ATLAS['Joker'] or G.ASSET_ATLAS['centers']
                    self.children.center = Sprite(self.T.x, self.T.y, self.T.w, self.T.h, atlas_ref, self.config.center.pos)
                else
        ...
        if _center.soul_pos then 
            local soul_atlas = G.ASSET_ATLAS[_center.atlas or 'Joker'] or G.ASSET_ATLAS['Joker']
            self.children.floating_sprite = Sprite(self.T.x, self.T.y, self.T.w, self.T.h, soul_atlas, self.config.center.soul_pos)
        ...
        if not self.children.back then
            local back_ref = (G.GAME and (G.GAME[self.back] or G.GAME.viewed_back or G.GAME.selected_back)) or nil
            local back_atlas = G.ASSET_ATLAS[(back_ref and back_ref.atlas) or 'centers'] or G.ASSET_ATLAS["centers"]
            self.children.back = Sprite(self.T.x, self.T.y, self.T.w, self.T.h, back_atlas, self.params.bypass_back or ((back_ref and back_ref.pos) or G.P_CENTERS['b_red'].pos))
...
function Card:set_ability(center, initial, delay_sprites)
    ...
    local subtle_additions_enabled = is_local_mod_enabled and is_local_mod_enabled('subtle_additions')
    ...
    if center.name == "Wee Joker" and (center.discovered or self.bypass_discovery_center) then 
        local wee_scale = subtle_additions_enabled and 0.3 or 0.7
        H = H*wee_scale
        W = W*wee_scale
        self.T.h = H
        self.T.w = W
    ...
    if subtle_additions_enabled and center.name == "Ancient Joker" and (center.discovered or self.bypass_discovery_center) then 
        H = H*1.1
        W = W*1.1
        self.T.h = H
        self.T.w = W
...
function Card:add_to_deck(from_debuff)
    ...
    if not self.added_to_deck then
        ...
        if self.ability.name == 'Chudjoker' then
            for k, v in pairs(G.GAME.probabilities) do
...
function Card:generate_UIBox_ability_table()
    ...
    elseif self.ability.set == 'Joker' then -- all remaining jokers
        ...
        elseif self.ability.name == 'Chudjoker' then
        elseif self.ability.name == 'Free Refill' then loc_vars = {self.ability.extra.xmult_gain, localize(G.GAME.current_round.j_minim_free_refill_card.suit, 'suits_singular'), self.ability.extra.xmult, colours = {G.C.SUITS[G.GAME.current_round.j_minim_free_refill_card.suit]}}
        elseif self.ability.name == 'RNA' then
        elseif self.ability.name == 'Flyer' then loc_vars = {self.ability.extra.money}
        elseif self.ability.name == 'Number One' then
        elseif self.ability.name == 'Chocolate Coin' then loc_vars = {self.ability.extra.money}
        elseif self.ability.name == 'Pestrica' then loc_vars = {self.ability.extra.Xmult}
        elseif self.ability.name == "Devil's Knife" then
            local spade_kings = 0
            if G.playing_cards then
                for _, playing_card in ipairs(G.playing_cards) do
                    if playing_card:get_id() == 13 and playing_card:is_suit('Spades') then spade_kings = spade_kings + 1 end
                end
            end
            loc_vars = {spade_kings}
        elseif self.ability.name == 'Power Creep' then loc_vars = {self.ability.extra.Xmult}
    ...
    -- Generic custom-center localization fallback for native mod content.
    if not loc_vars and self.config and self.config.center then
        local center = self.config.center
        local in_collection = self.area and self.area.config and self.area.config.collection
        local can_run_dynamic_loc = (self.area and self.area.config and not in_collection) or
            (in_collection and (center._native_smods or center._mikas_native))

        local function normalize_dynamic_loc_vars(out)
            ...
            local vars = out.vars ~= nil and out.vars or out
            if vars == nil then return nil end
            if type(vars) ~= 'table' then vars = {vars} end
            if next(vars) == nil then return nil end

            if out.colours then
                vars.colours = out.colours
            end

            return vars
        end

        if can_run_dynamic_loc and type(center.loc_vars) == 'function' then
            local ok, out = pcall(center.loc_vars, center, {}, self)
            if ok then
                loc_vars = normalize_dynamic_loc_vars(out)
            end
        end

        if can_run_dynamic_loc and (not loc_vars) and type(center.loc_def) == 'function' then
            local ok, out = pcall(center.loc_def, self)
            if not ok then
                ok, out = pcall(center.loc_def, center, {}, self)
            end
            if not ok then
                ok, out = pcall(center.loc_def, center, {})
            end
            if ok then
                loc_vars = normalize_dynamic_loc_vars(out)
            end
...
function Card:calculate_dollar_bonus()
    ...
    if self.ability.set == "Joker" then
        ...
        if self.ability.name == 'Chocolate Coin' then
            ...
            if self.ability.extra.money <= 0 then
                G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.6, blocking = false,
                    func = function()
                        ...
                        card_eval_status_text(self, 'extra', nil, nil, nil, {message = localize('k_eaten_ex')})
                        G.E_MANAGER:add_event(Event({
                            trigger = 'after',
                            delay = 0.3,
                            blockable = false,
                ...
                G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.3, blocking = false,
                    func = function()
                        self:juice_up(0.3, 0.4)
                        card_eval_status_text(self, 'extra', nil, nil, nil, {message = "-$1"})
                        return true
                    end
                }))
            end
...
function Card:open()
    if self.ability.set == "Booster" then
        ...
        local is_web_version = G.IS_WEB
        local pack_size_num = math.floor(tonumber(self.ability.extra) or 2)
        local pack_name_str = tostring(self.ability.name)
        local card_x = self.T.x
        local card_y = self.T.y
        local pack_cards = {}
        local safe_ante = (G and G.GAME and G.GAME.round_resets and G.GAME.round_resets.ante) or 1
        
        G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.4, func = function()
            self:explode()

            local _card_creation_attempts = 0
            G.E_MANAGER:add_event(Event({trigger = 'after', delay = 1.3*math.sqrt(G.SETTINGS.GAMESPEED), blockable = false, blocking = false, func = function()
                _card_creation_attempts = _card_creation_attempts + 1
                if not G.pack_cards then
                    return _card_creation_attempts > 120 and true or false
                end
                
                if not pack_size_num or pack_size_num <= 0 then
                    return true
                end
                
                local target_area = G.pack_cards
                if not target_area then
                    return true
                end

                if is_web_version then
                    collectgarbage('step', 240)
                end
                
                for i = 1, pack_size_num do
                    local success, card_or_err = pcall(function()
                        local new_card = nil
                        local realtime_ms = math.floor(((G.TIMERS and G.TIMERS.REAL) or love.timer.getTime()) * 1000)
                        local pack_seed_suffix = '_' .. tostring(safe_ante) .. '_' .. tostring(realtime_ms) .. '_' .. tostring(i)
                        
                        if pack_name_str:find('Buffoon') then
                            new_card = create_card("Joker", target_area, nil, nil, true, true, nil, 'buf'..pack_seed_suffix)
                        elseif pack_name_str:find('Arcana') then 
                            if G.GAME.used_vouchers.v_omen_globe and pseudorandom('omen_globe'..pack_seed_suffix) > 0.8 then
                                new_card = create_card("Spectral", target_area, nil, nil, true, true, nil, 'ar2'..pack_seed_suffix)
                            else
                                new_card = create_card("Tarot", target_area, nil, nil, true, true, nil, 'ar1'..pack_seed_suffix)
                            end
                        elseif pack_name_str:find('Celestial') then
                            if G.GAME.used_vouchers.v_telescope and i == 1 then
                                ...
                                new_card = create_card("Planet", target_area, nil, nil, true, true, _planet, 'pl1'..pack_seed_suffix)
                            else
                                new_card = create_card("Planet", target_area, nil, nil, true, true, nil, 'pl1'..pack_seed_suffix)
                            end
                        elseif pack_name_str:find('Spectral') then
                            new_card = create_card("Spectral", target_area, nil, nil, true, true, nil, 'spe'..pack_seed_suffix)
                        elseif pack_name_str:find('Standard') then
                            new_card = create_card((pseudorandom(pseudoseed('stdset'..safe_ante..pack_seed_suffix)) > 0.6) and "Enhanced" or "Base", target_area, nil, nil, nil, true, nil, 'sta'..pack_seed_suffix)
                            local edition_rate = 2
                            if new_card then
                                local edition = poll_edition('standard_edition'..safe_ante..pack_seed_suffix, edition_rate, true)
                                new_card:set_edition(edition)
                                local seal_rate = 10
                                local seal_poll = pseudorandom(pseudoseed('stdseal'..safe_ante..pack_seed_suffix))
                                if seal_poll > 1 - 0.02*seal_rate then
                                    local seal_type = pseudorandom(pseudoseed('stdsealtype'..safe_ante..pack_seed_suffix))
                                    if seal_type > 0.75 then new_card:set_seal('Red')
                                    elseif seal_type > 0.5 then new_card:set_seal('Blue')
                                    elseif seal_type > 0.25 then new_card:set_seal('Gold')
                                    else new_card:set_seal('Purple')
                                    end
                                end
                            end
                        end
                    ...
                    if not success then
                        local fallback_ok, fallback_card = pcall(function()
                            return create_card("Base", target_area, nil, nil, nil, true, nil, 'sta_fallback_'..tostring(i)..'_'..tostring(math.floor(((G.TIMERS and G.TIMERS.REAL) or love.timer.getTime())*1000)))
                        end)
                        if fallback_ok and fallback_card then
                            fallback_card.T.x = card_x
                            fallback_card.T.y = card_y
                            fallback_card:start_materialize({G.C.WHITE, G.C.WHITE}, nil, 1.5*G.SETTINGS.GAMESPEED)
                            pack_cards[i] = fallback_card
                        end
                    else
                        local card = card_or_err
                        if not card then
                            local fallback_ok, fallback_card = pcall(function()
                                return create_card("Base", target_area, nil, nil, nil, true, nil, 'sta_nilfallback_'..tostring(i)..'_'..tostring(math.floor(((G.TIMERS and G.TIMERS.REAL) or love.timer.getTime())*1000)))
                            end)
                            if fallback_ok and fallback_card then
                                fallback_card.T.x = card_x
                                fallback_card.T.y = card_y
            ...
            local _emplace_attempts = 0
            G.E_MANAGER:add_event(Event({trigger = 'after', delay = 1.3*math.sqrt(G.SETTINGS.GAMESPEED) + (pack_size_num * 0.15), blockable = false, blocking = false, func = function()
                _emplace_attempts = _emplace_attempts + 1
                if G.pack_cards then 
                    if G.pack_cards.VT and G.ROOM and G.pack_cards.VT.y < G.ROOM.T.h then 
                        for k, v in ipairs(pack_cards) do
                            G.pack_cards:emplace(v)
                        end
                        return true
                    else
                        return _emplace_attempts > 120 and true or false
                    end
                else
                    return _emplace_attempts > 120 and true or false
                end
        ...
        G.E_MANAGER:add_event(Event({trigger = 'after', delay = 2.0*math.sqrt(G.SETTINGS.GAMESPEED), func = function()
            self:remove()
            return true
        end}))
...
function Card:explode(dissolve_colours, explode_time_fac)
    ...
    -- Skip ease events on web to avoid ref_table = self metamethod issues
    if not G.IS_WEB then
        ...
        -- Web fallback: just set dissolve directly after delay
        G.E_MANAGER:add_event(Event({
            trigger = 'after',
            blockable = false,
            delay = 0.9*explode_time,
    ...
    G.E_MANAGER:add_event(Event({
        ...
        func = (function()
            ...
            -- Skip ease event on web to avoid ref_table = self metamethod issues
            if not G.IS_WEB then
                G.E_MANAGER:add_event(Event({
                    trigger = 'ease',
                    blockable = false,
                ...
                -- Web fallback: just set dissolve directly
                G.E_MANAGER:add_event(Event({
                    trigger = 'after',
                    blockable = false,
                    delay = 0.1*explode_time,
...
function Card:calculate_joker(context)
    ...
    if self.ability.set == "Joker" and not self.debuff then
        if self.config and self.config.center and type(self.config.center.calculate) == 'function' then
            local ok_modded, modded_result = pcall(self.config.center.calculate, self, context)
            if ok_modded and modded_result ~= nil then
        ...
        if context.open_booster then
            if self.ability.name == 'Flyer' then
        ...
        elseif context.cards_destroyed then
            if self.ability.name == 'Free Refill' and not context.blueprint then
                local suit_cards = 0
                ...
                if suit_cards > 0 then
                    self.ability.extra.xmult = self.ability.extra.xmult + suit_cards*self.ability.extra.xmult_gain
                    return {
        ...
        elseif context.remove_playing_cards then
            if self.ability.name == 'Free Refill' and not context.blueprint then
                ...
                if suit_cards > 0 then
                    ...
                    G.E_MANAGER:add_event(Event({
                        func = function()
                            card_eval_status_text(self, 'extra', nil, nil, nil, {message = localize('k_upgrade_ex'), colour = G.C.XMULT})
                            return true
                        end
                    }))
                end
        ...
        elseif context.end_of_round then
            ...
            elseif not context.blueprint then
                ...
                if self.ability.name == 'Pirouette' and context.game_over == false and context.main_eval and not context.blueprint then
                    self.ability.extra.invis_rounds = self.ability.extra.invis_rounds + 1
                    if self.ability.extra.invis_rounds == self.ability.extra.total_rounds then
                        local eval = function(card) return not card.REMOVED end
                        juice_card_until(self, eval, true)
                    end
                    return {
                        message = (self.ability.extra.invis_rounds <= self.ability.extra.total_rounds) and (self.ability.extra.invis_rounds .. '/' .. self.ability.extra.total_rounds) or localize('k_active_ex'),
                        colour = G.C.FILTER
                    }
                end
        ...
        elseif context.individual then
            if context.cardarea == G.play then
                ...
                if self.ability.name == 'Pirouette' and self.ability.extra.current_effect == 3 and not context.blueprint then
                    context.other_card:set_base(G.P_CARDS.S_K)
        ...
        elseif context.repetition then
            if context.cardarea == G.play then
                ...
                if self.ability.name == 'Number One' and G.GAME.current_round.hands_played == 0 then
                    return {
                        message = localize('k_again_ex'),
                ...
                if self.ability.name == 'Pirouette' and self.ability.extra.current_effect == 2 then
                    return {
                        message = localize('k_again_ex'),
                        repetitions = self.ability.extra.repetitions,
                        card = self
        ...
        elseif context.other_joker then
            ...
            if context.cardarea == G.jokers then
                if context.before then
                    ...
                    if self.ability.name == 'Pestrica' and #context.scoring_hand == 4 and not context.blueprint then
                    ...
                    if self.ability.name == 'Pirouette' and not context.blueprint then
                        self.ability.extra.current_effect = pseudorandom('CHAOS effect', self.ability.extra.random_eff1, self.ability.extra.random_eff2)
                        if self.ability.extra.current_effect == 3 then
                            return {
                                message = 'ALL KINGS!',
                                colour = G.C.PURPLE,
                                card = self
                    ...
                    if self.ability.name == 'RNA' and G.GAME.current_round.hands_left == 0 then
                        if #context.full_hand == 1 then
                            G.playing_card = (G.playing_card and G.playing_card + 1) or 1
                            local _card = copy_card(context.full_hand[1], nil, nil, G.playing_card)
                ...
                elseif context.after then
                    ...
                    if self.ability.name == 'Seltzer' and not context.blueprint then
                        ...
                        if self.ability.name == 'Free Refill' and self.ability.extra.xmult > 1 then
                            return {
                                message = localize{type='variable',key='a_xmult',vars={self.ability.extra.xmult}},
                                colour = G.C.XMULT,
                                Xmult_mod = self.ability.extra.xmult
                        ...
                        if self.ability.name == 'Pestrica' and self.ability.extra.Xmult > 1 then
                            return {
                                message = localize{type='variable',key='a_xmult',vars={self.ability.extra.Xmult}},
                                colour = G.C.XMULT,
                                Xmult_mod = self.ability.extra.Xmult
                        ...
                        if self.ability.name == 'Power Creep' then
                            if context.setting_blind and not context.blueprint then
                                self.ability.extra.event_added = false
                            ...
                            if context.joker_main then
                                if not context.blueprint and G.GAME and G.GAME.blind and G.GAME.blind.boss and G.GAME.chips and G.GAME.blind.chips and G.GAME.blind.chips > 0 and (not self.ability.extra.event_added) and (G.GAME.chips / G.GAME.blind.chips >= 3) then
                                    local counter = 0
                                    if G.jokers and G.jokers.cards then
                                        for _, joker_card in ipairs(G.jokers.cards) do
                                            if joker_card and joker_card.ability and joker_card.ability.name == 'Power Creep' then
                                                counter = counter + 1
                                            end
                                        end
                                    end
                                    ...
                                    if counter > 0 then
                                        G.E_MANAGER:add_event(Event({
                                            func = function()
                                                for i = 1, counter do
                                                    local new_card = create_card('Joker', G.jokers, nil, nil, nil, nil, 'j_mod_power_creep')
                                                    if new_card then
                                    ...
                                    self.ability.extra.event_added = true
                                end
                        ...
                        if self.ability.name == 'Pirouette' then
                            if self.ability.extra.current_effect == 1 then
                                local temp_xmult = pseudorandom('CHAOS mult', self.ability.extra.Xmin, self.ability.extra.Xmax)
                                return {
                                    message = 'X CHAOS MULT',
                                    colour = G.C.PURPLE,
                                    Xmult_mod = temp_xmult
                                }
                            elseif self.ability.extra.current_effect == 4 then
                                return {
                                    message = 'BYE-BYE!',
                                    colour = G.C.PURPLE
                                }
                            elseif self.ability.extra.current_effect == 5 then
                                return {
                                    message = 'CUT IN HALF!',
                                    colour = G.C.PURPLE,
                                    Xmult_mod = self.ability.extra.half_mult
                                }
                            end
                        end
                        if self.ability.name == "Devil's Knife" then
                            local spade_kings = 0
                            if G.playing_cards then
                                for _, playing_card in ipairs(G.playing_cards) do
                                    if playing_card:get_id() == 13 and playing_card:is_suit('Spades') then spade_kings = spade_kings + 1 end
                            ...
                            if spade_kings > 0 then
                                return {
                                    message = localize{type='variable',key='a_xmult',vars={spade_kings}},
                                    colour = G.C.MULT,
                                    Xmult_mod = spade_kings
                                }
                            end
                        end
                        ...
                        if self.ability.name == 'Bull' and (G.GAME.dollars + (G.GAME.dollar_buffer or 0)) > 0 then
                            local subtle_bull_sound = (is_local_mod_enabled and is_local_mod_enabled('subtle_additions')) and 'multhit1' or nil
                            return {
            ...
            if self.ability.name == 'Pirouette' and (self.ability.extra.invis_rounds >= self.ability.extra.total_rounds) and not context.blueprint then
...
function Card:update(dt)
    ...
    if G.STAGE == G.STAGES.RUN then
        ...
        if self.ability.name == 'Ramen' and self.area == G.jokers and is_local_mod_enabled and is_local_mod_enabled('subtle_additions') then
            self.T.h = 2.7512195121951 * (self.ability.x_mult / 2)
            self.T.w = 2.0487804878049 * (self.ability.x_mult / 2)
        end
