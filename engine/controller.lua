...
function Controller:set_cursor_position()
    ...
    if self.HID.mouse or self.HID.touch then 
        ...
        -- For web version, prefer cursor_position set by mousemoved callback for better accuracy
        if not (self.cursor_position.x and self.cursor_position.y) then
...
function Controller:update(dt)
    ...
    if not self.cursor_down.handled then 
        ...
        elseif G and G.SETTINGS and G.SETTINGS.enable_drag_select and #self.collision_list == 0 then
    ...
    if not self.cursor_up.handled then 
        ...
        if self.cursor_down.target then 
            ...
            -- Mobile-like swipe gestures on drag release for card actions.
            if G and G.QOL_FLAGS and G.QOL_FLAGS.mobile_like_dragging and self.HID.touch and self.dragging.prev_target and getmetatable(self.dragging.prev_target) == Card then
                local dy = ((self.cursor_up.T and self.cursor_up.T.y) or 0) - ((self.cursor_down.T and self.cursor_down.T.y) or 0)
                if dy < -0.45 and type(self.dragging.prev_target.swipe_up) == 'function' then
                    pcall(function() self.dragging.prev_target:swipe_up() end)
                elseif dy > 0.45 and type(self.dragging.prev_target.swipe_down) == 'function' then
                    pcall(function() self.dragging.prev_target:swipe_down() end)
                end
            end
        end
    ...
    if self.dragSelectActive and self.dragSelectActive.active then
        local closest, best_dist = nil, math.huge
        for i = 1, #self.collision_list do
            local v = self.collision_list[i]
            local cur_dist = Vector_Dist(self.cursor_hover.T, v.T)
            if v.area and v.area.config and v.area.config.type == 'hand' and v.states.hover.can and (not v.states.drag.is) and cur_dist < best_dist then
                closest = v
                best_dist = cur_dist
            end
        end
    ...
    if self.dragging.target and (not self.dragging.handled) and self.cursor_down.duration and (self.cursor_down.duration > (0.15 * G.SPEEDFACTOR)) then
        if G and G.QOL_FLAGS and G.QOL_FLAGS.mobile_like_dragging and type(create_drag_target_from_card) == 'function' then
            pcall(function()
    ...
    if self.is_cursor_down then
        ...
        self.cursor_down.duration = G.TIMERS.TOTAL - self.cursor_down.time
        if self.cursor_up.target then self.cursor_up.target = nil end
    else
        if self.cursor_down.target then self.cursor_down.target = nil end
