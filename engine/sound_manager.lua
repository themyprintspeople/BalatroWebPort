...
for _, filename in ipairs(sound_files) do
    ...
    for i = 1, 1 do
        if extension == '.ogg' then
            ...
            -- Ensure key exists so music keys are discoverable by MODULATE/RESTART_MUSIC
            SOURCES[sound_code] = SOURCES[sound_code] or {}
            -- Create audio source with a safe fallback: streaming inside a thread can fail on some platforms
            local mode = (string.find(sound_code, 'music') and "stream" or 'static')
            local ok, src = pcall(function()
                return love.audio.newSource("resources/sounds/"..filename, mode)
            end)
            if not ok or not src then
                -- Fallback to static if streaming failed (streaming is sometimes unsupported inside threads)
                local ok2, src2 = pcall(function()
                    return love.audio.newSource("resources/sounds/"..filename, 'static')
                end)
                src = ok2 and src2 or nil
            end
            if not src then
                LOAD_CHANNEL:push('audio load failed - '..filename)
            else
                local s = {
                    sound = src,
...
function PLAY_SOUND(args)
    ...
    local mode = should_stream and "stream" or 'static'
    local ok, src = pcall(function()
        return love.audio.newSource("resources/sounds/"..args.sound_code..'.ogg', mode)
    end)
    if not ok or not src then
        -- Try fallback to static mode
        local ok2, src2 = pcall(function()
    ...
    if not src then
        LOAD_CHANNEL:push('audio create failed - '..tostring(args.sound_code))
        return nil
    end
    local s = {sound = src}
