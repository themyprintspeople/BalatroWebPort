...
function card_eval_status_text(card, eval_type, amt, percent, dir, extra)
    ...
    elseif eval_type == 'extra' or eval_type == 'jokers' then 
        sound = extra.sound or (extra.edition and 'foil2' or extra.mult_mod and 'multhit1' or extra.Xmult_mod and 'multhit2' or 'generic1')
        if extra.edition then 
            colour = G.C.DARK_EDITION
        end
        volume = extra.volume or (extra.edition and 0.3 or sound == 'multhit2' and 0.7 or 1)
...
function change_shop_size(mod)
    ...
    if G.shop_jokers and G.shop_jokers.cards then
        ...
        G.shop:recalculate()
        if mod > 0 then
            -- Incrementally add shop jokers in batches to avoid blocking the main thread (browser)
            ...
            if to_make > 0 then
                ...
                G.E_MANAGER:add_event(Event({
                    ...
                    func = function()
                        local work = 0
                        while made < to_make and work < batch do
                            made = made + 1
                            G.shop_jokers:emplace(create_card_for_shop(G.shop_jokers))
                            work = work + 1
                        end
                        if made >= to_make then return true end
                        return false
                    end
                }))
            end
...
function get_deck_from_name(_name)
    ...
    if type(_name) == 'table' and _name.set == 'Back' then
        return _name
    end

    if type(_name) == 'string' then
        local by_key = G.P_CENTERS[_name]
        if by_key and by_key.set == 'Back' then
            return by_key
        end

        for _, v in pairs(G.P_CENTERS) do
            if v and v.set == 'Back' and v.name == _name then
                return v
            end
        end
    end
...
function get_next_voucher_key(_from_tag)
    ...
    -- Add ante and run seed to voucher selection for variety
    local seed_append = (G.GAME and G.GAME.pseudorandom and G.GAME.pseudorandom.seed) and ('_'..G.GAME.pseudorandom.seed) or ''
    _pool_key = _pool_key .. '_ante' .. (G.GAME.round_resets.ante or 1) .. seed_append
    if _from_tag then _pool_key = _pool_key .. '_fromtag' end
...
function get_next_tag_key(append)
    ...
    print("[DEBUG] Tag pool key: ".._pool_key.." (append: "..tostring(append)..")")
    local seed_value = pseudoseed(_pool_key)
    print("[DEBUG] Tag pseudoseed value: "..tostring(seed_value))
    local _tag = pseudorandom_element(_pool, seed_value)
    local it = 1
    while _tag == 'UNAVAILABLE' do
        it = it + 1
        _tag = pseudorandom_element(_pool, pseudoseed(_pool_key..'_resample'..it))
    end
    print("[DEBUG] Tag selected: "..tostring(_tag))
...
-- Helper function to convert seed string to number by concatenating alphabet positions
local function seed_string_to_number(str)
    if type(str) ~= "string" then return str or 0 end
    local result = ""
    for i = 1, #str do
        local char = str:sub(i, i)
        local byte = string.byte(char)
        -- A=65, Z=90 -> A=1, B=2... Z=26
        if byte >= 65 and byte <= 90 then
            result = result .. tostring(byte - 64)
        -- a=97, z=122 -> a=1, b=2... z=26
        elseif byte >= 97 and byte <= 122 then
            result = result .. tostring(byte - 96)
        -- 0=48, 9=57 -> 0=0, 1=1... 9=9
        elseif byte >= 48 and byte <= 57 then
            result = result .. tostring(byte - 48)
        end
    end
    return tonumber(result) or 0
end

-- Counter for pack generation to ensure each pack gets a unique seed
local pack_generation_counter = 0

function get_pack(_key, _type)
    local pack_debug = G and (G.PACK_DEBUG or G.DEBUG)
    -- First pack is always Buffoon
    if not G.GAME.first_shop_buffoon and not G.GAME.banned_keys['p_buffoon_normal_1'] then
        G.GAME.first_shop_buffoon = true
        if pack_debug then
            print("[PACK DEBUG] First pack - returning Buffoon")
        end
        return G.P_CENTERS['p_buffoon_normal_'..(math.random(1, 2))]
    end
    
    -- Increment counter for each pack generation
    pack_generation_counter = pack_generation_counter + 1

    -- Use a deterministic hashed value instead of reseeding math.random every call.
    -- Reseeding the global RNG repeatedly can cause biased or repeatable results
    -- when seeds are similar (very likely when called multiple times per frame).
    local frame_seed = (G.TIMERS and G.TIMERS.REAL) or love.timer.getTime()
    local game_seed_raw = (G.GAME and G.GAME.pseudorandom and G.GAME.pseudorandom.seed) or 0
    -- Compose a string and hash it to get a pseudo-random value in [0,1]
    local hash_input = tostring(frame_seed) .. '|' .. tostring(game_seed_raw) .. '|' .. tostring(pack_generation_counter)
    local random_val = pseudohash(hash_input)

    if pack_debug then
        print("[PACK DEBUG] counter=" .. pack_generation_counter .. " frame_seed=" .. tostring(frame_seed) .. " game_seed_raw=" .. tostring(game_seed_raw) .. " hash_input=" .. hash_input .. " random_val=" .. tostring(random_val))
    end
    
    -- Random pack selection after first Buffoon
    ...
    if pack_debug then
        print("[PACK DEBUG] Total cumulative weight: " .. cume)
    end

    -- Use random value for variety
    local poll = random_val * cume

    if pack_debug then
        print("[PACK DEBUG] Poll value: " .. tostring(poll))
        -- Dump pool entries and running cumulative for debugging
        local running = 0
        for k, v in ipairs(G.P_CENTER_POOLS['Booster']) do
            if (not _type or _type == v.kind) and not G.GAME.banned_keys[v.key] then
                running = running + (v.weight or 1)
                print(string.format("[PACK DEBUG] pool[%d]=%s kind=%s weight=%s running=%s", k, tostring(v.key), tostring(v.kind), tostring(v.weight or 1), tostring(running)))
            else
                print(string.format("[PACK DEBUG] pool[%d]=%s SKIPPED (kind=%s banned=%s)", k, tostring(v.key), tostring(v.kind), tostring( not not G.GAME.banned_keys[v.key])) )
            end
        end
    end
    ...
    for k, v in ipairs(G.P_CENTER_POOLS['Booster']) do
        if not G.GAME.banned_keys[v.key] then 
            ...
            if it >= poll and it - (v.weight or 1) <= poll then 
                center = v
                if pack_debug then
                    print("[PACK DEBUG] Selected pack: " .. v.key .. " (kind=" .. v.kind .. ", weight=" .. v.weight .. ")")
                end
                break 
            end
        end
...
function get_current_pool(_type, _rarity, _legendary, _append)
        ...
        if _type == 'Joker' then 
            ...
            if pack_debug then
                print("[RARITY DEBUG] Raw pseudorandom value: " .. tostring(rarity) .. " for seed 'rarity" .. tostring(G.GAME.round_resets.ante) .. tostring(_append or '') .. "'")
            end
            rarity = (_legendary and 4) or (rarity > 0.95 and 3) or (rarity > 0.7 and 2) or 1
            if pack_debug then
                print("[RARITY DEBUG] Joker rarity roll: " .. tostring(rarity) .. " (legendary=" .. tostring(_legendary) .. ", _rarity=" .. tostring(_rarity) .. ", _append=" .. tostring(_append) .. ")")
            end
...
function reset_minim_free_refill_card()
    ...
    if valid_cards[1] then
        local refill_card = pseudorandom_element(valid_cards, pseudoseed('2cas'..G.GAME.round_resets.ante))
        G.GAME.current_round.j_minim_free_refill_card.suit = refill_card.base.suit
    end
end
...
function get_new_boss()
    ...
    -- Debug: print eligible bosses
    local boss_list = {}
    for k, v in pairs(eligible_bosses) do
        table.insert(boss_list, k)
    end
    print("[DEBUG] Eligible bosses: "..table.concat(boss_list, ", "))
    
    -- Use hashed seed for better randomization across different runs
    local boss_seed_key = 'boss_select_ante_'..G.GAME.round_resets.ante..'_'..G.GAME.pseudorandom.seed
    local _, boss = pseudorandom_element(eligible_bosses, pseudoseed(boss_seed_key))
    print("[DEBUG] Boss selected: "..tostring(boss).." for ante "..tostring(G.GAME.round_resets.ante).." seed_key: "..boss_seed_key)
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    local looks_like_lowlight = _c and (
        _c.key == 'p_lowlight_normal' or
        _c.atlas == 'p_lowlight_normal' or
        (type(_c.name) == 'string' and _c.name:find('Cigarette', 1, true))
    )
    if looks_like_lowlight then
        -- Normalize center metadata so tooltip resolution always hits Low Light keys.
        _c.key = 'p_lowlight_normal'
        _c.set = 'Booster'
        _c.name = _c.name or 'Cigarette Pack...?'
    ...
    -- Hard safety path: Low Light booster hover has modded localization shape and can nil-crash
    -- in the generic booster description pipeline. Build a minimal safe tooltip and return.
    if _c and _c.set == 'Booster' and (
        _c.key == 'p_lowlight_normal' or
        _c.name == 'Low Light Cigarette Pack' or
        _c.atlas == 'p_lowlight_normal' or
        (type(_c.name) == 'string' and _c.name:find('Cigarette', 1, true))
    ) then
        local choose = (_c.config and _c.config.choose) or 1
        local extra = (_c.config and _c.config.extra) or 3
        local desc_scale = 0.32 * ((G and G.LANG and G.LANG.font and G.LANG.font.DESCSCALE) or 1)
        local name_scale = 0.5 * ((G and G.LANG and G.LANG.font and G.LANG.font.NAMESCALE) or 1)
        ...
        local safe_nodes = full_UI_table.main
        if type(safe_nodes) ~= 'table' then
            safe_nodes = {}
            full_UI_table.main = safe_nodes
        end
        ...
        safe_nodes[1] = {
            {
                n = G.UIT.T,
                config = {
                    text = 'Choose ' .. tostring(choose) .. ' of up to',
        ...
        safe_nodes[2] = {
            {
                n = G.UIT.T,
                config = {
                    text = tostring(extra) .. ' X-Playing Cards',
                    colour = G.C.FILTER,
                    scale = desc_scale,
                },
            },
    ...
    elseif _c.set == 'Booster' then 
        ...
        if is_lowlight then desc_override = 'p_lowlight_normal'; loc_vars = {(_c.config and _c.config.choose) or 1, (_c.config and _c.config.extra) or 3}
        elseif _c.name == 'Arcana Pack' then desc_override = 'p_arcana_normal'; loc_vars = {_c.config.choose, _c.config.extra}
        ...
        if is_lowlight then
            local safe_name_nodes = (type(full_UI_table.name) == 'table') and full_UI_table.name or {}
            local ok_name, resolved_name = pcall(function()
                return localize{type = 'name', set = 'Other', key = name_override, nodes = safe_name_nodes}
            end)
            if ok_name and resolved_name then
                full_UI_table.name = resolved_name
            elseif not full_UI_table.name then
                full_UI_table.name = true
            end
        elseif not full_UI_table.name then
        ...
        if is_lowlight then
            local safe_desc_nodes = (type(desc_nodes) == 'table') and desc_nodes or {}
            local ok_desc = pcall(function()
                localize{type = 'other', key = desc_override, nodes = safe_desc_nodes, vars = loc_vars}
            end)
            if (not ok_desc) and type(desc_nodes) == 'table' then
                localize{type = 'other', key = 'p_lowlight_normal', nodes = desc_nodes, vars = {(_c.config and _c.config.choose) or 1, (_c.config and _c.config.extra) or 3}}
            end
        else
