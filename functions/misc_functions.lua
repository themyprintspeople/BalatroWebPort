-- Safe channel push helper for web compatibility
function safe_channel_push(manager, channel_name, data)
    if manager and manager[channel_name] then
        manager[channel_name]:push(data)
        return true
    end
    return false
end
...
function pseudoshuffle(list, seed)
  ...
  -- Use Fisher-Yates shuffle with seed-based random values
  if seed then
    -- Use seed directly to generate deterministic but unique shuffle
    for i = #list, 2, -1 do
      -- Generate a pseudo-random value based on seed and index
      local random_val = ((seed * 1000 + i) * 9301 + 49297) % 233280
      local j = math.floor(random_val % i) + 1
      list[i], list[j] = list[j], list[i]
    end
  else
    -- Standard random shuffle when no seed
    for i = #list, 2, -1 do
...
function generate_starting_seed()
  if G.GAME.stake >= 8 then
    ...
    if r_tally > 0 and g_tally > 0 then
      ...
      while not seed_found do
        ...
        -- Add frame count and real time for web randomness
        local random_factor = (G.FRAMES and G.FRAMES.DRAW or 0) + (love.timer and love.timer.getTime() or 0) * 1000
        seed_found = random_string(8, extra_num + G.CONTROLLER.cursor_hover.T.x*0.33411983 + G.CONTROLLER.cursor_hover.T.y*0.874146 + 0.412311010*G.CONTROLLER.cursor_hover.time + random_factor)
  ...
  -- Add frame count and real time for web randomness
  local random_factor = (G.FRAMES and G.FRAMES.DRAW or 0) + (love.timer and love.timer.getTime() or 0) * 1000
  return random_string(8, G.CONTROLLER.cursor_hover.T.x*0.33411983 + G.CONTROLLER.cursor_hover.T.y*0.874146 + 0.412311010*G.CONTROLLER.cursor_hover.time + random_factor)
end
...
function pseudorandom_element(_t, seed)
  ...
  -- Use seed value directly to pick index instead of math.random()
  local index
  if seed then
    -- Convert seed to index: multiply by count and add 1
    index = math.floor(seed * #keys) + 1
    -- Clamp to valid range
    if index > #keys then index = #keys end
    if index < 1 then index = 1 end
  else
    index = math.random(#keys)
  end
  
  local key = keys[index].k
  -- debug removed: pseudorandom element
  return _t[key], key 
end

function random_string(length, seed)
  -- Use a local seeded PRNG when a seed is provided so we don't reseed the global RNG
  local rand
  if seed then
    local function make_lcg(s)
      -- convert fractional seed to 32-bit integer
      local val = math.floor((s % 1) * 4294967296)
      if val == 0 then val = math.floor((s * 1000003) % 4294967296) end
      return function()
        val = (1664525 * val + 1013904223) % 4294967296
        return val / 4294967296
      end
    end
    rand = make_lcg(seed)
    -- burn a few values for better distribution like original code intended
    for k = 1, 3 do rand() end
  else
    rand = math.random
  end
  ...
  for i = 1, length do
    local r = rand()
    if r > 0.7 then
      ret = ret .. string.char(rand_range(string.byte('1'), string.byte('9')))
    elseif r > 0.45 then
      ret = ret .. string.char(rand_range(string.byte('A'), string.byte('N')))
    else
      ret = ret .. string.char(rand_range(string.byte('P'), string.byte('Z')))
    end
  end
...
function pseudoseed(key, predict_seed)
  ...
  local result = (G.GAME.pseudorandom[key] + (G.GAME.pseudorandom.hashed_seed or 0))/2
  -- debug removed: pseudoseed
  return result
end

function pseudorandom(seed, min, max)
  if type(seed) == 'string' then seed = pseudoseed(seed) end
  -- Use a local seeded PRNG to avoid reseeding the global RNG
  local function make_lcg(s)
    local val = math.floor((s % 1) * 4294967296)
    if val == 0 then val = math.floor((s * 1000003) % 4294967296) end
    return function()
  ...
  local rand = make_lcg(seed)
  -- Burn a few values like original
  for i = 1, 3 do rand() end
  if min and max then
...
function play_sound(sound_code, per, vol)
  ...
  if sound_code and G.SETTINGS.SOUND.volume > 0.001 then
    ...
    G.ARGS.play_sound.music_control = G.SETTINGS.SOUND.music_control
    ...
    if G.F_SOUND_THREAD then
      ...
      -- Wrap in pcall for web version to prevent audio errors from crashing
      local success, err = pcall(PLAY_SOUND, G.ARGS.play_sound)
      if not success and G.IS_WEB then
        -- Silently ignore audio errors on web
      elseif not success then
        print("[ERROR] play_sound failed: " .. tostring(err))
      end
    end
  end
end
...
function modulate_sound(dt)
  ...
  -- Debug: print desired music track and effective music volume
  -- removed verbose audio debug print
...
function send_score(_score)
  if G.F_HTTP_SCORES and G.SETTINGS.COMP and G.F_STREAMER_EVENT and G.HTTP_MANAGER and G.HTTP_MANAGER.out_channel then
...
function process_save_request(request)
  if not request or not request.type then return end

  local function flush_fs()
    if love and love.filesystem and love.filesystem.flush then
      pcall(love.filesystem.flush)
    end
  end

  if request.type == 'save_progress' and request.save_progress then
    local profile_num = request.save_progress.SETTINGS and request.save_progress.SETTINGS.profile or 1
    ...
    local _append = false
    for k, v in pairs(request.save_progress.UDA or {}) do
    ...
    if request.save_progress.SETTINGS then
  ...
  elseif request.type == 'save_settings' then
    if request.save_settings then
      compress_and_save('settings.jkr', request.save_settings)
    end
    if request.profile_num and request.save_profile then
      local prefix_profile = tostring(request.profile_num)
      if not love.filesystem.getInfo(prefix_profile) then love.filesystem.createDirectory(prefix_profile) end
      compress_and_save(prefix_profile..'/profile.jkr', request.save_profile)
    end

  elseif request.type == 'save_metrics' and request.save_metrics then
  ...
  elseif request.type == 'save_run' and request.save_table then
    local prefix_profile = tostring(request.profile_num or 1)
    if not love.filesystem.getInfo(prefix_profile) then love.filesystem.createDirectory(prefix_profile) end
    prefix_profile = prefix_profile..'/'
...
function refresh_local_mod_toggle_cache()
  local toggles = {}
...
function is_local_mod_enabled(mod_id)
  if type(mod_id) ~= 'string' then return false end
  if type(G.ACTIVE_MOD_TOGGLES) ~= 'table' then
    refresh_local_mod_toggle_cache()
  end
  return G.ACTIVE_MOD_TOGGLES[mod_id] == true
end

function init_localization()
  if is_local_mod_enabled('subtle_additions') and G.localization and G.localization.descriptions then
    local joker_desc = G.localization.descriptions.Joker or {}
    local blind_desc = G.localization.descriptions.Blind or {}
    local tag_desc = G.localization.descriptions.Tag or {}

    joker_desc.j_clever = {name = 'Pesky Joker', text = {'{C:chips}+#1#{} Chips if played', 'hand contains', 'a {C:attention}#2#'}}
    joker_desc.j_credit_card = {name = 'Dedit Carb', text = {'Go up to', '{C:red}-$#1#{} in debt', '{C:inactive}erm... crebit?'}}
    joker_desc.j_family = {
      name = 'The Family (Guy)',
      text = {
    ...
    joker_desc.j_green_joker = {
      name = 'The Green Grinner',
      text = {
    ...
    blind_desc.bl_eye = {name = 'The I', text = {'No repeat hand', 'types this round'}}
    blind_desc.bl_goad = {name = 'The Chode', text = {'All Spade cards', 'are debuffed'}}
    blind_desc.bl_mark = {name = 'The Mark(iplier)', text = {'All face cards are', 'drawn face down'}}

    tag_desc.tag_economy = {
      name = 'Buzzfeed',
      text = {
        'Doubles your money',
...
function PLAY_SOUND(args)
  -- Return early if audio is muted
  if G.F_MUTE then return nil end
  
  args.per = args.per or 1
  args.vol = args.vol or 1
  SOURCES[args.sound_code] = SOURCES[args.sound_code] or {}

  -- For web version with lazy loading, check if we should reuse existing source
  if G.F_LAZY_AUDIO and #SOURCES[args.sound_code] > 0 then
    -- Try to find an available (stopped) source to reuse
    for i = #SOURCES[args.sound_code], 1, -1 do
      local existing = SOURCES[args.sound_code][i]
      if existing and existing.sound and not existing.sound:isPlaying() then
        -- Reuse this source
        existing.sound:stop()
        existing.sound:seek(0)
        SET_SFX(existing, args)
        pcall(function() love.audio.play(existing.sound) end)
        return existing
      end
    end
  end
  ...
  -- Web version: use static mode for everything (streaming can be unreliable in browsers)
  if G.F_LAZY_AUDIO then
    should_stream = false
    -- removed music load debug print
  end
  
  -- Web-safe audio source creation with error handling
  local success, result = pcall(function()
    return love.audio.newSource("resources/sounds/"..args.sound_code..'.ogg', should_stream and "stream" or 'static')
  end)
  
  if not success then
    if G.F_LAZY_AUDIO then
      print("[WARN] Failed to load audio for: " .. args.sound_code .. " (web version)")
      return nil
    end
    print("[WARN] Failed to create audio source for: " .. args.sound_code .. " (attempting fallback)")
    -- Attempt fallback to static mode
    local ok2, res2 = pcall(function()
      return love.audio.newSource("resources/sounds/"..args.sound_code..'.ogg', 'static')
    end)
    if not ok2 or not res2 then
      print("[ERROR] Fallback audio creation failed for: " .. args.sound_code)
      return nil
    end
    result = res2
  end
  
  local s = {sound = result}
  table.insert(SOURCES[args.sound_code], s)
  s.sound_code = args.sound_code
  s.created_time = args.time or G.TIMERS.REAL
  ...
  -- Web-safe audio playback
  -- Debug: log creation and playback attempts for music tracks
  -- created log removed
  local okplay, err = pcall(function() love.audio.play(s.sound) end)
  if not okplay and s.sound and s.sound.play then
    pcall(function() s.sound:play() end)
  end
  -- playback errors handled silently; debug prints removed
  return s
end

function STOP_AUDIO()
  -- Return early if audio is muted (web version)
  if G.F_MUTE then return end
  
  for _, source in pairs(SOURCES) do
      for _, s in pairs(source) do
          -- Wrap in pcall for web version to prevent audio buffer errors
          pcall(function()
...
function SET_SFX(s, args)
  ...
  if string.find(s.sound_code,'music') then 
      ...
      pcall(function() s.sound:setVolume(s.current_volume*s.original_volume*(args.sound_settings.volume/100.0)*(args.sound_settings.music_volume/100.0)) end)
      pcall(function() s.sound:setPitch(s.original_pitch*args.pitch_mod) end)
  else
      if s.temp_pitch ~= s.original_pitch then 
          pcall(function() s.sound:setPitch(s.original_pitch) end)
    ...
    if sound_vol <= 0 then
      ...
      pcall(function() s.sound:setVolume(sound_vol) end)
    end
  end
end
...
function MODULATE(args)
  ...
  -- If the desired music track isn't present in SOURCES (common on web builds with lazy loading),
  -- attempt to create/play it once. This covers the case where SOURCES has no music keys at all.
  if args.desired_track and args.desired_track ~= '' and (not SOURCES[args.desired_track] or #SOURCES[args.desired_track] == 0) then
    -- Populate the sound code and try to PLAY_SOUND; ignore errors for web fallback
    local try_args = {
      sound_code = args.desired_track,
      per = args.per or 1,
      vol = args.vol or 1,
      pitch_mod = args.pitch_mod or args.pitch_mod,
      state = args.state,
      sound_settings = args.sound_settings,
      overlay_menu = args.overlay_menu,
      splash_vol = args.splash_vol,
    }
    pcall(function() PLAY_SOUND(try_args) end)
  end

  -- Ensure desired music is created for web lazy-loading: when SOURCES has no music keys
  -- browsers often require creation/play after a user gesture; create desired track if missing
  -- lazy-creation logic left in place, debug prints removed
  
  for k, v in pairs(SOURCES) do
      if (string.find(k,'music') and (args.desired_track ~= '')) then
          local is_playing = false
          pcall(function()
              is_playing = v[1] and v[1].sound and v[1].sound:isPlaying()
          end)
          -- Debug: when desired_track is music, report the state of each music source
          -- removed detailed modulate debug prints
            if is_playing then
            else
              if G.IS_WEB and v[1] and (((args.time or 0) - (v[1].created_time or (args.time or 0))) < 1.5) then
              -- On web, decoding/unlock can delay playback start; avoid immediate restart thrash.
              else
              RESTART_MUSIC(args)
              end
              break;
  ...
  for k, v in pairs(SOURCES) do
    ...
    while i <= #v do
        local is_playing = false
        pcall(function()
            is_playing = v[i].sound and v[i].sound:isPlaying()
        end)
      if not is_playing then
        if G.IS_WEB and (((args.time or 0) - (v[i].created_time or (args.time or 0))) < 1.5) then
          i = i + 1
        else
          table.remove(v, i)
        end
    ...
    for i, s in ipairs(v) do
      local is_playing = false
      pcall(function()
          is_playing = s.sound and s.sound:isPlaying()
      end)
      if is_playing and s.original_volume then
          SET_SFX(s, args)
      end
    end
  end
...
function RESTART_MUSIC(args)
  ...
  for k, v in pairs(SOURCES) do
      if string.find(k,'music') then 
          for i, s in ipairs(v) do
              pcall(function() 
                  if s.sound then s.sound:stop() end
              end)
          end
          SOURCES[k] = {}
          args.per = 0.7
          ...
          if not s then
            -- failed to create music; continue without verbose logging
          else
            s.initialized = true
          end
      end
