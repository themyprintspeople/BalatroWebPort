...
function Game:init()
    ...
    -- Detect if running in web version (love.js/emscripten)
    self.IS_WEB = love.system.getOS() == "Web"
    G.IS_WEB = self.IS_WEB  -- Also set as global for use in misc_functions
    
    self:set_globals()
    
    -- CRITICAL: Disable threading for web to prevent futex deadlocks
    if self.IS_WEB then
        print("[DEBUG] Web version detected - disabling threads, using lazy audio loading")
        self.F_SOUND_THREAD = false
        self.F_HTTP_SCORES = false
        self.F_MUTE = false  -- Enable audio but load lazily
        self.F_LAZY_AUDIO = true  -- Flag for lazy audio loading
    end
end

function Game:start_up()
    print("[DEBUG] Game:start_up() BEGIN")
    --Load the settings file
    print("[DEBUG] Loading settings...")
    local settings = get_compressed('settings.jkr')
    local settings_ver = nil
    if settings then 
        print("[DEBUG] Settings file found, unpacking...")
    ...
    print("[DEBUG] Boot timer: settings")
    ...
    SOURCES = {}
    if not self.IS_WEB then
        -- Desktop: Load all sounds upfront
        print("[DEBUG] Creating sound sources...")
        ...
        print("[DEBUG] Sound sources created: " .. tostring(table_length(SOURCES)))
    else
        -- Web: Create empty table, sounds will be loaded on-demand
        print("[DEBUG] Web version - audio will be lazy-loaded on demand")
    end

    self.SETTINGS.language = self.SETTINGS.language or 'en-us'
    print("[DEBUG] Language set to: " .. self.SETTINGS.language)
    boot_timer('settings', 'window init', 0.2)
    print("[DEBUG] Calling init_window...")
    self:init_window()
    print("[DEBUG] init_window COMPLETE")
    ...
    -- DISABLE THREADS FOR WEB - they cause futex deadlocks
    if not self.IS_WEB then
        G.SAVE_MANAGER = {
        ...
        print("[DEBUG] Skipping save manager thread (web version)")
        G.SAVE_MANAGER = {
            thread = nil,
            channel = {
                pop = function() return nil end,
                push = function(request)
                    if process_save_request then process_save_request(request) end
                end
            }
        }
    end
    ...
    if not self.IS_WEB then
        ...
        print("[DEBUG] Skipping HTTP manager thread (web version)")
        G.HTTP_MANAGER = {
            thread = nil,
            out_channel = {push = function() end},  -- Mock channel
            in_channel = {pop = function() return nil end}  -- Mock channel
        }
    end
...
function Game:init_item_prototypes()
    ...
    self.P_CENTERS = {
        ...
        j_chudjoker=        {order = 151,  unlocked = true, discovered = true, blueprint_compat = true, perishable_compat = true, eternal_compat = true, rarity = 3, cost = 6, name = "Chudjoker", pos = {x=0,y=0}, atlas = 'JS_Chud', set = "Joker", config = {}},
        j_minim_free_refill={order = 152,  unlocked = true, discovered = true, blueprint_compat = true, perishable_compat = true, eternal_compat = true, rarity = 3, cost = 7, name = "Free Refill", pos = {x=0,y=0}, atlas = 'JS_LordMinimus', set = "Joker", config = {extra = {xmult = 1, xmult_gain = 0.5}}},
        j_minim_rna=        {order = 153,  unlocked = true, discovered = true, blueprint_compat = true, perishable_compat = true, eternal_compat = true, rarity = 2, cost = 5, name = "RNA", pos = {x=1,y=0}, atlas = 'JS_LordMinimus', set = "Joker", config = {}},
        j_minim_flyer=      {order = 154,  unlocked = true, discovered = true, blueprint_compat = true, perishable_compat = true, eternal_compat = true, rarity = 1, cost = 6, name = "Flyer", pos = {x=2,y=0}, atlas = 'JS_LordMinimus', set = "Joker", config = {extra = {money = 1}}},
        j_minim_number_one= {order = 155,  unlocked = true, discovered = true, blueprint_compat = true, perishable_compat = true, eternal_compat = true, rarity = 2, cost = 6, name = "Number One", pos = {x=3,y=0}, atlas = 'JS_LordMinimus', set = "Joker", config = {extra = {repetitions = 1}}},
        j_minim_chocolate_coin={order = 156,unlocked = true, discovered = true, blueprint_compat = true, perishable_compat = true, eternal_compat = true, rarity = 1, cost = 6, name = "Chocolate Coin", pos = {x=0,y=1}, atlas = 'JS_LordMinimus', set = "Joker", config = {extra = {money = 6}}},
        j_pesj_pestrica=   {order = 157,  unlocked = true, discovered = true, blueprint_compat = true, perishable_compat = true, eternal_compat = true, rarity = 4, cost = 20, name = "Pestrica", pos = {x=0,y=0}, soul_pos = {x=0,y=1}, atlas = 'JS_Pestrica', set = "Joker", config = {extra = {Xmult = 1, Xmult_gain = 0.5}}},
        j_jevi_jevil=       {order = 158,  unlocked = true, discovered = true, blueprint_compat = true, perishable_compat = false, eternal_compat = true, rarity = 3, cost = 8, name = "Pirouette", pos = {x=0,y=0}, atlas = 'JS_Jevil', set = "Joker", config = {extra = {Xmin = 3, Xmax = 8, invis_rounds = 0, total_rounds = 4, random_eff1 = 1, random_eff2 = 5, repetitions = 3, half_chips = 0.5, half_mult = 0.5, current_effect = 1}}, no_pool_flag = 'jevil_is_sold'},
        j_jevi_devilsknife= {order = 159,  unlocked = true, discovered = true, blueprint_compat = true, perishable_compat = true, eternal_compat = true, rarity = 4, cost = 20, name = "Devil's Knife", pos = {x=1,y=0}, soul_pos = {x=2,y=0}, atlas = 'JS_Jevil', set = "Joker", config = {extra = {suit = 'Spades', Xmult = 0}}, yes_pool_flag = 'jevil_is_sold'},
        j_mod_power_creep=  {order = 160,  unlocked = true, discovered = true, blueprint_compat = true, perishable_compat = true, eternal_compat = true, rarity = 3, cost = 10, name = "Power Creep", pos = {x=0,y=0}, atlas = 'JS_PowerCreep', set = "Joker", config = {extra = {Xmult = 1.5}}},
        ...
        b_js_buffoon=       {name = "Buffoon Deck",     stake = 1, unlocked = true, order = 17, pos = {x=0,y=5}, set = "Back", config = {booster = 'p_buffoon_jumbo_1'}, discovered = true},
    ...
    local function read_local_mod_toggles_for_native()
        local out = {}
    ...
    local native_mod_toggles = read_local_mod_toggles_for_native()
    local function is_native_mod_enabled(mod_id)
        return native_mod_toggles[mod_id] == true
    end

    if type(apply_qol_native_mod_toggles) == 'function' then
        pcall(function()
            apply_qol_native_mod_toggles(native_mod_toggles)
        end)
    end

    pcall(function()
        if type(init_mor_jokers_native_content) == 'function' and is_native_mod_enabled('mor_jokers') then
            init_mor_jokers_native_content()
        end
    end)

    pcall(function()
        if type(init_jank_jonklers_native_content) == 'function' and is_native_mod_enabled('jank_jonklers') then
            init_jank_jonklers_native_content()
        end
    end)

    pcall(function()
        if type(init_mikas_native_content) == 'function' and is_native_mod_enabled('mikas_mod_collection') then
            init_mikas_native_content()
        end
    end)

    local function collect_native_mod_keys(mod_id)
        if type(get_native_mod_center_keys) == 'function' then
            return get_native_mod_center_keys(mod_id)
        end
        return {}
    end

    local function apply_native_mod_toggles()
        local toggles = native_mod_toggles
        local map = {
            buffoon_deck = {'b_js_buffoon'},
            mikas_mod_collection = type(get_mikas_native_center_keys) == 'function' and get_mikas_native_center_keys() or {},
            mor_jokers = collect_native_mod_keys('mor_jokers'),
            jank_jonklers = collect_native_mod_keys('jank_jonklers'),
        ...
        for mod_id, center_keys in pairs(map) do
            local enabled = toggles[mod_id] == true
            for _, center_key in ipairs(center_keys) do
                local center = self.P_CENTERS[center_key]
                if center then
    ...
    for k, v in pairs(self.P_CENTERS) do
        v.key = k
        if v.set == 'Joker' and not v.omit then table.insert(self.P_CENTER_POOLS['Joker'], v) end
        ...
        if not v.wip then 
            ...
            if v.rarity and v.set == 'Joker' and not v.demo and not v.omit then table.insert(self.P_JOKER_RARITY_POOLS[v.rarity], v) end
        end
    end
...
function Game:set_language()
    ...
    if localization ~= nil then
      ...
      init_localization()
            ...
            if type(apply_mikas_native_localization) == 'function' then
...
function Game:set_render_settings()
    ...
    local mod_toggles = {}
    local joker_atlas_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/Jokers.png"
    local blind_chips_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/BlindChips.png"
    local shop_sign_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/ShopSignAnimation.png"
    local cards_1_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/8BitDeck.png"
    local cards_2_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/8BitDeck_opt2.png"
    local centers_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/Enhancers.png"
    local tarot_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/Tarots.png"
    local voucher_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/Vouchers.png"
    local booster_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/boosters.png"
    local icons_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/icons.png"
    local tags_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/tags.png"
    do
        local toggle_path = 'webbridge/local_mod_toggles.txt'
        if love.filesystem.getInfo(toggle_path) then
            local raw = love.filesystem.read(toggle_path) or ''
            raw = raw:gsub('\r', '')
            for line in string.gmatch(raw, '([^\n]+)') do
                local id, state = line:match('^([^\t]+)\t([01])$')
                if id and state then
                    mod_toggles[id] = (state == '1')
                end
            end
        end
    end

    self.ACTIVE_MOD_TOGGLES = mod_toggles
    G.ACTIVE_MOD_TOGGLES = mod_toggles

    if type(apply_qol_native_mod_toggles) == 'function' then
        pcall(function()
            apply_qol_native_mod_toggles(mod_toggles)
        end)
    end

    if mod_toggles.themed_jokers then
        joker_atlas_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/themed_jokers.png"
        tarot_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/themed_tarots.png"
    end

    if mod_toggles.subtle_additions then
        joker_atlas_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/subtle_jokers.png"
        blind_chips_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/subtle_blind_chips.png"
        shop_sign_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/subtle_shop_sign.png"
        cards_1_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/subtle_deck_1.png"
        cards_2_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/subtle_deck_2.png"
        centers_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/subtle_enhancers.png"
        tarot_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/subtle_tarots.png"
        voucher_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/subtle_vouchers.png"
        booster_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/subtle_boosters.png"
        icons_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/subtle_icons.png"
        tags_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/subtle_tags.png"
    end

    if mod_toggles.baldatro then
        joker_atlas_path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/bald_jokers.png"
    end
    ...
    --spritesheets
    self.animation_atli = {
        {name = "blind_chips", path = blind_chips_path,px=34,py=34, frames = 21},
        {name = "shop_sign", path = shop_sign_path,px=113,py=57, frames = 4}
    }
    self.asset_atli = {
        {name = "cards_1", path = cards_1_path,px=71,py=95},
        {name = "cards_2", path = cards_2_path,px=71,py=95},
        {name = "centers", path = centers_path,px=71,py=95},
        {name = "Joker", path = joker_atlas_path,px=71,py=95},
        {name = "Tarot", path = tarot_path,px=71,py=95},
        {name = "Voucher", path = voucher_path,px=71,py=95},
        {name = "Booster", path = booster_path,px=71,py=95},
        ...
        {name = 'icons', path = icons_path,px=66,py=66},
        {name = 'tags', path = tags_path,px=34,py=34},
        ...
        {name = 'JS_Buffoon', path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/buffoon_deck.png",px=71,py=95},
        {name = 'JS_HighCard', path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/high_card_deck.png",px=71,py=95},
        {name = 'JS_Chud', path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/chudjoker_jokers.png",px=71,py=95},
        {name = 'JS_LordMinimus', path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/lord_minimus_atlas.png",px=71,py=95},
        {name = 'JS_Pestrica', path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/pestrica_jokers.png",px=71,py=95},
        {name = 'JS_Jevil', path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/jevil_jokers.png",px=71,py=95},
        {name = 'JS_PowerCreep', path = "resources/textures/"..self.SETTINGS.GRAPHICS.texture_scaling.."x/powercreep-jokers.png",px=71,py=95},
    ...
    if type(append_native_smods_asset_atlases) == 'function' then
        append_native_smods_asset_atlases(self.asset_atli)
    end
    if type(append_mikas_native_asset_atlases) == 'function' then
        append_mikas_native_asset_atlases(self.asset_atli)
    end

    --Load in all atli defined above
    local use_mipmaps = not self.IS_WEB -- Disable mipmaps for web version to avoid WebGL errors
    print("[DEBUG] Loading textures... use_mipmaps=" .. tostring(use_mipmaps) .. ", IS_WEB=" .. tostring(self.IS_WEB))
    
    print("[DEBUG] Loading " .. #self.animation_atli .. " animation atlases...")
    for i=1, #self.animation_atli do
        print("[DEBUG] Loading animation atlas " .. i .. "/" .. #self.animation_atli .. ": " .. self.animation_atli[i].name)
        ...
        self.ANIMATION_ATLAS[self.animation_atli[i].name].image = love.graphics.newImage(self.animation_atli[i].path, {mipmaps = use_mipmaps, dpiscale = self.SETTINGS.GRAPHICS.texture_scaling})
    ...
    print("[DEBUG] Animation atlases loaded")

    print("[DEBUG] Loading " .. #self.asset_atli .. " asset atlases...")
    for i=1, #self.asset_atli do
        print("[DEBUG] Loading asset atlas " .. i .. "/" .. #self.asset_atli .. ": " .. self.asset_atli[i].name)
        self.ASSET_ATLAS[self.asset_atli[i].name] = {}
        self.ASSET_ATLAS[self.asset_atli[i].name].name = self.asset_atli[i].name
        self.ASSET_ATLAS[self.asset_atli[i].name].image = love.graphics.newImage(self.asset_atli[i].path, {mipmaps = use_mipmaps, dpiscale = self.SETTINGS.GRAPHICS.texture_scaling})
    ...
    print("[DEBUG] Asset atlases loaded")
    for i=1, #self.asset_images do
        ...
        self.ASSET_ATLAS[self.asset_images[i].name].image = love.graphics.newImage(self.asset_images[i].path, {mipmaps = use_mipmaps, dpiscale = 1})
...
function Game:save_notify(card)
    if not G.SAVE_MANAGER or not G.SAVE_MANAGER.channel then return end
...
function Game:splash_screen()
    ...
    -- Force skip splash on web due to rendering issues
    if G.SETTINGS.skip_splash == 'Yes' or self.IS_WEB then 
        print("[DEBUG] Skipping splash screen (IS_WEB=" .. tostring(self.IS_WEB) .. ")")
        G:main_menu()
        return 
    end
...
function Game:init_game_object()
    ...
    return {
        ...
        current_round = {
            ...
            j_minim_free_refill_card = {suit = 'Spades'},
            hands_left = 0,
            hands_played = 0,
            discards_left = 0,
            discards_used = 0,
...
function Game:start_run(args)
    print("[DEBUG] start_run() BEGIN")
    args = args or {}

    local saveTable = args.savetext or nil
    G.SAVED_GAME = nil

    print("[DEBUG] start_run() calling prep_stage...")
    self:prep_stage(G.STAGES.RUN, saveTable and saveTable.STATE or G.STATES.BLIND_SELECT)
    print("[DEBUG] start_run() prep_stage complete")
    ...
    local selected_back =
        (saveTable and saveTable.BACK and (saveTable.BACK.key or saveTable.BACK.name)) or
        (args.challenge and args.challenge.deck and args.challenge.deck.type) or
        (self.GAME.viewed_back and self.GAME.viewed_back.effect and self.GAME.viewed_back.effect.center and (self.GAME.viewed_back.effect.center.key or self.GAME.viewed_back.name)) or
        (self.GAME.selected_back and self.GAME.selected_back.effect and self.GAME.selected_back.effect.center and (self.GAME.selected_back.effect.center.key or self.GAME.selected_back.name)) or
        'Red Deck'
    ...
    if not saveTable then
        ...
        print("[DEBUG] Game seed generated: " .. tostring(self.GAME.pseudorandom.seed))
    end
    ...
    if not saveTable then
        ...
        local use_forced_voucher = (G.SETTINGS.tutorial_progress and not G.SETTINGS.tutorial_complete and G.SETTINGS.tutorial_progress.forced_voucher)
        local use_forced_tags = (G.SETTINGS.tutorial_progress and not G.SETTINGS.tutorial_complete and G.SETTINGS.tutorial_progress.forced_tags)
        self.GAME.current_round.voucher = use_forced_voucher or get_next_voucher_key()
        self.GAME.round_resets.blind_tags.Small = (use_forced_tags and use_forced_tags[1]) or get_next_tag_key('small_ante'..self.GAME.round_resets.ante..'_seed_'..self.GAME.pseudorandom.seed)
        self.GAME.round_resets.blind_tags.Big = (use_forced_tags and use_forced_tags[2]) or get_next_tag_key('big_ante'..self.GAME.round_resets.ante..'_seed_'..self.GAME.pseudorandom.seed)
    else
    ...
    if saveTable then 
        ...
        local deferred_card_area_loads = {
            shop_jokers = true,
            shop_booster = true,
            shop_vouchers = true,
        }
        for k, v in pairs(cardAreas) do
            ...
            G['load_'..k] = v
            if not deferred_card_area_loads[k] then
        ...
        if not card_protos then 
            card_protos = {}
            print("[DEBUG] Building deck from P_CARDS, count: "..#G.P_CARDS)
            print("[DEBUG] _de = "..tostring(_de))
            if _de then
                print("[DEBUG] _de.yes_ranks = "..tostring(_de.yes_ranks))
                print("[DEBUG] _de.no_ranks = "..tostring(_de.no_ranks))
                print("[DEBUG] _de.yes_suits = "..tostring(_de.yes_suits))
                print("[DEBUG] _de.no_suits = "..tostring(_de.no_suits))
            end
            for k, v in pairs(self.P_CARDS) do
                ...
                if keep then 
                    ...
                    print("[DEBUG] Filtered out card: "..k.." keep="..tostring(keep))
                end
            end
            print("[DEBUG] Final card_protos count: "..#card_protos)
        end 
        ...
        -- Randomize the initial order before creating cards to avoid deterministic shuffle results
        pseudoshuffle(card_protos, pseudoseed('deck_init'))

        for k, v in ipairs(card_protos) do
            card_from_control(v)
        end
        print("[DEBUG] start_run() cards created: " .. #G.playing_cards)

        self.GAME.starting_deck_size = #G.playing_cards
    end

    print("[DEBUG] start_run() adding delay event...")
    delay(0.5)
    print("[DEBUG] start_run() delay event added")

    if not saveTable then
        print("[DEBUG] start_run() initializing round...")
        G.GAME.current_round.discards_left = G.GAME.round_resets.discards
        G.GAME.current_round.hands_left = G.GAME.round_resets.hands
        print("[DEBUG] start_run() shuffling deck...")
        self.deck:shuffle()
        print("[DEBUG] start_run() shuffle complete")
        ...
        reset_ancient_card()
        reset_castle_card()
        reset_minim_free_refill_card()
        print("[DEBUG] start_run() round init complete")
    end

    print("[DEBUG] start_run() creating blind...")
    G.GAME.blind = Blind(0,0,2, 1)
    print("[DEBUG] start_run() aligning cards...")
    self.deck:align_cards()
    self.deck:hard_set_cards()
    print("[DEBUG] start_run() cards aligned")
...
function Game:update(dt)
    ...
    if G.HTTP_MANAGER and G.HTTP_MANAGER.in_channel then
...
function Game:draw()
    ...
    -- Web safety: Log every 100 frames only when debugging
    if self.IS_WEB and G.DEBUG and G.FRAMES.DRAW % 100 == 0 then
        print("[DRAW] Frame " .. G.FRAMES.DRAW .. " | STATE=" .. tostring(G.STATE))
    end
    ...
    -- Draw virtual cursor BEFORE the regular cursor (so it's under CRT effect)
    if IS_WEB and G.VIRTUAL_CURSOR and G.VIRTUAL_CURSOR.enabled then
        love.graphics.push('all')
        love.graphics.setColor(1, 1, 1, 1)
        local cx, cy = G.VIRTUAL_CURSOR.x, G.VIRTUAL_CURSOR.y
        
        if G.VIRTUAL_CURSOR.image then
            local scale = G.VIRTUAL_CURSOR.scale or 0.5
            local hotspot_x = 1 * scale
            local hotspot_y = 3 * scale
            love.graphics.draw(G.VIRTUAL_CURSOR.image, cx - hotspot_x, cy - hotspot_y, 0, scale, scale)
        else
            -- Fallback cursor
            love.graphics.setLineWidth(2)
            love.graphics.setColor(0, 0, 0, 1)
            love.graphics.polygon('fill', cx, cy, cx + 3, cy + 8, cx + 6, cy + 16, cx + 8, cy + 20, cx - 2, cy + 20, cx - 4, cy + 16, cx - 3, cy + 8)
            love.graphics.setColor(1, 1, 1, 1)
            love.graphics.polygon('fill', cx, cy + 1, cx + 2, cy + 8, cx + 4, cy + 16, cx + 6, cy + 19, cx - 1, cy + 19, cx - 3, cy + 16, cx - 2, cy + 8)
...
love.graphics.pop()
    ...
    -- Usage overlay (toggled in settings)
    if G.SETTINGS.show_usage then
        love.graphics.push()
        love.graphics.setColor(1, 1, 1, 1)
        local fps = love.timer.getFPS()
        local ram = collectgarbage("count") / 1024
        love.graphics.print("FPS: " .. fps, 10, 10)
        love.graphics.print("RAM: " .. string.format("%.2f", ram) .. " MB", 10, 30)
        love.graphics.pop()
    end
                            ...
                            if math.abs(G.shop.T.y - G.shop.VT.y) < 3 then
                                ...
                                if not shop_exists then
                                    ...
                                    if G.load_shop_jokers then 
                                        ...
                                        -- Create shop cards incrementally to avoid freezing the main thread in web builds
                                        do
                                            local to_make = G.GAME.shop.joker_max - #G.shop_jokers.cards
                                            if to_make > 0 then
                                                local made_count = 0
                                                local batch = G.IS_WEB and 2 or 4
                                                G.E_MANAGER:add_event(Event({
                                                    trigger = 'after',
                                                    delay = 0,
                                                    blockable = false,
                                                    ...
                                                    func = function()
                                                        local work = 0
                                                        while made_count < to_make and work < batch do
                                                            made_count = made_count + 1
                                                            G.shop_jokers:emplace(create_card_for_shop(G.shop_jokers))
                                                            work = work + 1
                                                        end
                                                        if made_count >= to_make then return true end
                                                        return false
                                                    end
                                                }))
                                            end
                                        ...
                                        if #booster_to_make > 0 then
                                            ...
                                            G.E_MANAGER:add_event(Event({
                                                ...
                                                func = function()
                                                    ...
                                                    if made >= #booster_to_make then
                                                        for j = 1, #G.GAME.tags do
                                                            G.GAME.tags[j]:apply_to_run({type = 'voucher_add'})
                                                        end
                                                        for j = 1, #G.GAME.tags do
                                                            G.GAME.tags[j]:apply_to_run({type = 'shop_final_pass'})
                                                        end
                                                        return true
                                                    end
                                                    return false
    ...
    if not G.STATE_COMPLETE then
        ...
        G.E_MANAGER:add_event(Event({
            ...
            func = function()
                ...
                if self.GAME and self.GAME.selected_back then
