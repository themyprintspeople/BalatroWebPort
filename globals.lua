...
function Game:set_globals()
    ...
    --             Time
    --||||||||||||||||||||||||||||||
    -- Use a counter for web compatibility (os.time() may be constant in browsers)
    -- Increment global counter each time to ensure different seeds
    _G.SEED_COUNTER = (_G.SEED_COUNTER or 0) + 1
    self.SEED = math.floor((love.timer and love.timer.getTime() or 0) * 1000000) + _G.SEED_COUNTER * 12345
    print("[DEBUG] Generated SEED: " .. self.SEED .. " (counter: " .. _G.SEED_COUNTER .. ")")
    self.TIMERS = {
        TOTAL=0,
        REAL = 0,
        REAL_SHADER = 0,
    ...
    self.SETTINGS = {
        ...
        GAMESPEED = 1,
        paused = false,
        mouse_sensitivity = 50,  -- Virtual cursor sensitivity (10-100)
        show_usage = false,
        SOUND = {
            volume = 50,
            music_volume = 100,
    ...
    --        CONSTANTS
    --||||||||||||||||||||||||||||||
    -- Increase click tolerance for web version due to mouse coordinate jitter
    self.MIN_CLICK_DIST = IS_WEB and 2.5 or 0.9
