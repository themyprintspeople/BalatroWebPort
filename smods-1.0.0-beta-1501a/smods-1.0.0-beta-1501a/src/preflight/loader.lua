...
function loadMods(modsDirectory)
    ...
    local function processDirectory(directory, depth, flags)
        ...
        for _, filename in ipairs(NFS.getDirectoryItems(directory)) do
            ...
            elseif filename:lower():match("%.lua$") then -- Check for legacy headers
                if rawget(_G, 'IS_WEB') then
                    sendWarnMessage(('Skipping legacy Lua mod file in web mode: %s'):format(filename), 'Loader')
                else
                if depth == 1 then
    ...
    local is_web_runtime = rawget(_G, 'IS_WEB') and true or false
    if is_web_runtime then
        sendWarnMessage('Web mode detected: external mod loading is disabled for stability', 'Loader')
        lovely_directories = {}
    else
