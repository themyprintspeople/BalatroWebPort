...
function G.UIDEF.card_h_popup(card)
	...
	if card and card.config and card.config.center == G.P_CENTERS.m_stone then card.xability = nil end

	-- Booster popup mutation is unstable in collection previews; use vanilla popup path.
	if card and card.ability and card.ability.set == "Booster" then
		return card_h_popup_OG(card)
	end
		...
		if card.ability.name == "Low Light Cigarette Pack" then
			...
			if card.ability_UIBox_table and 
			   card.ability_UIBox_table.name then
			   	if G.localization and G.localization.descriptions and G.localization.descriptions.Other and G.localization.descriptions.Other["p_lowlight_normal"] then
	...
	if card.xability and card.ability_UIBox_table then 
		local AUT = card.ability_UIBox_table
		AUT.badges = AUT.badges or {}
		AUT.info = AUT.info or {}
		AUT.name = AUT.name or {}
		AUT.main = AUT.main or {}

		AUT.badges.xplaying = nil
		if AUT.badges and AUT.badges.xplaying == nil then
			--badge
			AUT.badges[#AUT.badges + 1] = "xplaying"
			--tooltip
			local tool_tip = {}
			...
			AUT.info[#AUT.info+1] = tool_tip
			--name
			if #AUT.name > 0 and AUT.name[#AUT.name] and AUT.name[#AUT.name].config then
				AUT.name = {AUT.name[#AUT.name]}
				AUT.name[#AUT.name].config.text = card.xability.name
			end
			local xcolour = hcm_determine_xplaying_colour(card)
			if xcolour and #AUT.name > 0 and AUT.name[#AUT.name] and AUT.name[#AUT.name].config then AUT.name[#AUT.name].config.colour = xcolour end
			...
			if xcard_name == "XPlayingHeartK" then 
				sendNestedMessage(AUT.main)
				sendInfoMessage(#AUT.main)
				if card.ability and card.ability.perma_mult and not card.debuff then
					local extra_mult = hcm_deep_cpy(AUT.main[1])
					if extra_mult and next(extra_mult) then
					...
					if AUT.main[#AUT.main] and AUT.main[#AUT.main][2] and AUT.main[#AUT.main][2].config and AUT.main[#AUT.main][2].config.text == hcm_misc_loc["extra_mult"] then
					else AUT.main[#AUT.main + 1] = extra_mult end
				end
			end
			...
			if G.GAME.hcm_san_galgano or xcard_name == "XPlayingSpadeK" then
				if G.GAME.hcm_disabled and G.GAME.hcm_disabled[xcard_name] then
					local cursed_text = hcm_deep_cpy(AUT.main[1])
					if cursed_text and cursed_text[1] and cursed_text[1].config then
						...
						if AUT.main[#AUT.main] and AUT.main[#AUT.main][1] and AUT.main[#AUT.main][1].config and AUT.main[#AUT.main][1].config.text == hcm_misc_loc["hcm_disabled"] then
						else AUT.main[#AUT.main + 1] = cursed_text end
					end
				end
			end
		end
       		...
       		func = function()
	            ...
	            G.E_MANAGER:add_event(Event({trigger = 'after', delay = 1.3*math.sqrt(G.SETTINGS.GAMESPEED), blockable = false, blocking = false, 
	            	func = function()
		                ...
		                local lowlight_joker_pool = {}
		                for hand_key, info in pairs(xplaying_jokers_info) do
		                    if xplaying_config[hand_key] and info and info.slug then
		                        local center_key = 'j_' .. tostring(info.slug)
		                        if G.P_CENTERS and G.P_CENTERS[center_key] then
		                            lowlight_joker_pool[#lowlight_joker_pool + 1] = center_key
		                        end
		                    end
		                end
		                ...
		                for i = 1, _size do
		                    ...
		                    if #lowlight_joker_pool > 0 then
		                        forced_key = pseudorandom_element(lowlight_joker_pool, pseudoseed('hcm_lowlight_joker_' .. tostring(i) .. '_' .. tostring(G.GAME.round_resets.ante)))
		                    end

		                    local card = create_card("Joker", G.pack_cards, nil, nil, nil, true, forced_key, 'hcmll')
		                    if card then
